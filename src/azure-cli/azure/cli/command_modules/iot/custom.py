...
from azure.cli.command_modules.iot.shared import (
    AuthenticationType,
    EncodingFormat,
    EndpointType,
    IdentityType,
...
def iot_dps_list(client, resource_group_name=None):
    ...
    return client.iot_dps_resource.list_by_resource_group(resource_group_name=resource_group_name)
...
def iot_dps_delete(client, dps_name, resource_group_name=None):
    ...
    return client.iot_dps_resource.begin_delete(
        provisioning_service_name=dps_name, resource_group_name=resource_group_name
    )
...
def iot_dps_policy_get(client, dps_name, access_policy_name, resource_group_name=None):
    ...
    return client.iot_dps_resource.list_keys_for_key_name(
        provisioning_service_name=dps_name, key_name=access_policy_name, resource_group_name=resource_group_name
    )
...
def iot_dps_policy_create(
    ...
    dps_access_policies.append(
        SharedAccessSignatureAuthorizationRuleAccessRightsDescription(
            key_name=access_policy_name,
            rights=access_policy_rights,
...
def iot_dps_linked_hub_get(cmd, client, dps_name, linked_hub, resource_group_name=None):
    ...
    raise ResourceNotFoundError(
        "Linked hub '{0}' does not exist. Use 'iot dps linked-hub show to see all linked hubs.".format(linked_hub)
    )
...
def iot_dps_linked_hub_create(
    ...
    dps = iot_dps_get(client, dps_name, resource_group_name)
    dps.properties.iot_hubs.append(
        IotHubDefinitionDescription(
...
def iot_dps_certificate_list(client, dps_name, resource_group_name=None):
    resource_group_name = _ensure_dps_resource_group_name(client, resource_group_name, dps_name)
    return client.dps_certificate.list(resource_group_name=resource_group_name, provisioning_service_name=dps_name)


def iot_dps_certificate_get(client, dps_name, certificate_name, resource_group_name=None):
    resource_group_name = _ensure_dps_resource_group_name(client, resource_group_name, dps_name)
    return client.dps_certificate.get(
        certificate_name=certificate_name, resource_group_name=resource_group_name, provisioning_service_name=dps_name
    )


def iot_dps_certificate_create(
    client, dps_name, certificate_name, certificate_path, resource_group_name=None, is_verified=None
):
    resource_group_name = _ensure_dps_resource_group_name(client, resource_group_name, dps_name)
    cert_list = client.dps_certificate.list(resource_group_name=resource_group_name, provisioning_service_name=dps_name)
    for cert in cert_list.value:
        if cert.name == certificate_name:
            raise CLIError(
                "Certificate '{0}' already exists. Use 'iot dps certificate update'"
...
def iot_dps_certificate_update(
    client, dps_name, certificate_name, certificate_path, etag, resource_group_name=None, is_verified=None
):
    resource_group_name = _ensure_dps_resource_group_name(client, resource_group_name, dps_name)
    cert_list = client.dps_certificate.list(resource_group_name=resource_group_name, provisioning_service_name=dps_name)
    for cert in cert_list.value:
...
def iot_hub_certificate_list(client, hub_name, resource_group_name=None):
    ...
    return client.certificates.list_by_iot_hub(resource_group_name=resource_group_name, resource_name=hub_name)


def iot_hub_certificate_get(client, hub_name, certificate_name, resource_group_name=None):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    return client.certificates.get(
        resource_group_name=resource_group_name, resource_name=hub_name, certificate_name=certificate_name
    )


def iot_hub_certificate_create(
    client, hub_name, certificate_name, certificate_path, resource_group_name=None, is_verified=None
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    # Get list of certs
    cert_list = client.certificates.list_by_iot_hub(resource_group_name=resource_group_name, resource_name=hub_name)
    for cert in cert_list.value:
        if cert.name == certificate_name:
            raise CLIError(
                "Certificate '{0}' already exists. Use 'iot hub certificate update'"
                " to update an existing certificate.".format(certificate_name)
            )
    certificate = open_certificate(certificate_path)
    if not certificate:
...
def iot_hub_certificate_update(
    client, hub_name, certificate_name, certificate_path, etag, resource_group_name=None, is_verified=None
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    cert_list = client.certificates.list_by_iot_hub(resource_group_name=resource_group_name, resource_name=hub_name)
    for cert in cert_list.value:
    ...
    raise CLIError(
        "Certificate '{0}' does not exist. Use 'iot hub certificate create' to create a new certificate.".format(
            certificate_name
        )
    )
...
def iot_hub_create(
    ...
    if enable_fileupload_notifications:
        if not fileupload_storage_connectionstring or not fileupload_storage_container_name:
            raise RequiredArgumentMissingError(
                "Please specify storage endpoint (storage connection string and storage container name)."
            )
    ...
    identity_based_file_upload = (
        fileupload_storage_authentication_type
        and fileupload_storage_authentication_type == AuthenticationType.IdentityBased.value
    )
    if not identity_based_file_upload and fileupload_storage_identity:
        raise RequiredArgumentMissingError(
            "In order to set a fileupload storage identity, please set file upload storage authentication (--fsa) to IdentityBased"
        )
    if identity_based_file_upload or fileupload_storage_identity:
        ...
        if fileupload_storage_identity in [None, SYSTEM_ASSIGNED_IDENTITY] and not system_identity:
            raise ArgumentUsageError(
                "System managed identity [--mi-system-assigned] must be enabled in order to use managed identity for file upload"
            )
        if (
            fileupload_storage_identity
            and fileupload_storage_identity != SYSTEM_ASSIGNED_IDENTITY
            and not user_identities
        ):
            raise ArgumentUsageError(
                "User identity [--mi-user-assigned] must be added in order to use it for file upload"
            )
    location = _ensure_location(cli_ctx, resource_group_name, location)
    ...
    event_hub_dic = {}
    event_hub_dic["events"] = EventHubProperties(retention_time_in_days=retention_day, partition_count=partition_count)
    feedback_Properties = FeedbackProperties(
        lock_duration_as_iso8601=timedelta(seconds=feedback_lock_duration),
        ttl_as_iso8601=timedelta(hours=feedback_ttl),
        max_delivery_count=feedback_max_delivery_count,
    )
    cloud_to_device_properties = CloudToDeviceProperties(
        max_delivery_count=c2d_max_delivery_count,
        default_ttl_as_iso8601=timedelta(hours=c2d_ttl),
        feedback=feedback_Properties,
    )
    msg_endpoint_dic = {}
    msg_endpoint_dic["fileNotifications"] = MessagingEndpointProperties(
        max_delivery_count=fileupload_notification_max_delivery_count,
    ...
    storage_endpoint_dic["$default"] = StorageEndpointProperties(
        ...
        identity=(
            ManagedIdentity(user_assigned_identity=fileupload_storage_identity) if fileupload_storage_identity else None
        ),
    )

    properties = IotHubProperties(
        event_hub_endpoints=event_hub_dic,
            ...
            if identity:
                ...
                if principal_id:
                    ...
                    for scope in identity_scopes:
                        assign_identity(
                            cmd.cli_ctx,
                            lambda: hub_description,
                            lambda hub: hub_description,
                            identity_role=identity_role,
                            identity_scope=scope,
                        )
        except HttpResponseError as e:
...
def iot_hub_get(cmd, client, hub_name, resource_group_name=None):
    ...
    if name_availability is not None and name_availability.name_available:
        raise CLIError(
            "An IotHub '{0}' under resource group '{1}' was not found.".format(hub_name, resource_group_name)
        )
...
def update_iot_hub_custom(
    ...
    if any(
        ...
        if not default_storage_endpoint:
            ...
            default_storage_endpoint = StorageEndpointProperties(
                container_name=fileupload_storage_container_name, connection_string=fileupload_storage_connectionstring
            )

        # if setting a fileupload storage identity or changing fileupload to identity-based
        if (
            fileupload_storage_identity
            or fileupload_storage_authentication_type == AuthenticationType.IdentityBased.value
        ):
...
def iot_hub_show_connection_string(
    client,
    hub_name=None,
    resource_group_name=None,
    ...
    if hub_name is None:
        ...
        def conn_str_getter(h):
            return _get_hub_connection_string(
                client, h.name, h.additional_properties["resourcegroup"], policy_name, key_type, show_all
            )
...
def _get_hub_connection_string(client, hub_name, resource_group_name, policy_name, key_type, show_all):
    ...
    return [
        conn_str_template.format(
            hostname, p.key_name, p.secondary_key if key_type == KeyType.secondary else p.primary_key
        )
        for p in policies
    ]
...
def iot_hub_consumer_group_create(
    client, hub_name, consumer_group_name, resource_group_name=None, event_hub_name="events"
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    consumer_group_body = EventHubConsumerGroupBodyDescription(
        properties=EventHubConsumerGroupName(name=consumer_group_name)
    )
...
def iot_hub_consumer_group_get(
    client, hub_name, consumer_group_name, resource_group_name=None, event_hub_name="events"
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
...
def iot_hub_consumer_group_delete(
    client, hub_name, consumer_group_name, resource_group_name=None, event_hub_name="events"
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    ...
    def setter(hub):
        ...
        if user_identities:
            for identity in user_identities:
                hub.identity.user_assigned_identities[identity] = (
                    hub.identity.user_assigned_identities.get(identity, {})
                    if hub.identity.user_assigned_identities
                    else {}
                )

        has_system_identity = hub.identity.type in [
        ...
        if system_identity or has_system_identity:
            hub.identity.type = (
                IdentityType.system_assigned_user_assigned.value
                if hub.identity.user_assigned_identities
                else IdentityType.system_assigned.value
            )
        else:
            hub.identity.type = (
                IdentityType.user_assigned.value if hub.identity.user_assigned_identities else IdentityType.none.value
            )

        poller = client.iot_hub_resource.begin_create_or_update(
            resource_group_name=resource_group_name, resource_name=hub_name, iot_hub_description=hub, if_match=hub.etag
        )
...
def iot_hub_identity_remove(
    cmd, client, hub_name, system_identity=None, user_identities=None, resource_group_name=None
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    hub = iot_hub_get(cmd, client, hub_name, resource_group_name)
    ...
    if system_identity:
        ...
        hub_identity.type = (
            ...
            if hub.identity.type in [IdentityType.user_assigned.value, IdentityType.system_assigned_user_assigned.value]
    ...
    if user_identities:
        ...
        for identity in user_identities:
            if not hub_identity.user_assigned_identities[identity]:
                raise ArgumentUsageError(
                    "Hub {0} is not currently using a user-assigned identity with id: {1}".format(hub_name, identity)
                )
    ...
    if hub_identity.type in [IdentityType.system_assigned.value, IdentityType.system_assigned_user_assigned.value]:
        hub_identity.type = (
            IdentityType.system_assigned_user_assigned.value
            if getattr(hub_identity, "user_assigned_identities", None)
            else IdentityType.system_assigned.value
        )
    else:
        hub_identity.type = (
...
def iot_hub_policy_key_renew(
    cmd, client, hub_name, policy_name, regenerate_key, resource_group_name=None, no_wait=False
):
    hub = iot_hub_get(cmd, client, hub_name, resource_group_name)
    policies = []
    policies.extend(iot_hub_policy_list(client, hub_name, hub.additional_properties["resourcegroup"]))
    ...
    updated_policies.append(
        SharedAccessSignatureAuthorizationRule(
            key_name=requested_policy[0].key_name,
...
def validate_authentication_type_input(
...
def iot_hub_routing_endpoint_create(
    cmd,
    client,
    hub_name,
    endpoint_name,
        ...
        hub.properties.routing.endpoints.event_hubs.append(
            RoutingEventHubProperties(
                ...
                identity=(
                    ...
                    if identity and identity not in [IdentityType.none.value, SYSTEM_ASSIGNED_IDENTITY]
                    else None
                ),
            )
        )
...
def iot_hub_routing_endpoint_delete(
    cmd, client, hub_name, endpoint_name=None, endpoint_type=None, resource_group_name=None
):
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    hub = iot_hub_get(cmd, client, hub_name, resource_group_name)
    hub.properties.routing.endpoints = _delete_routing_endpoints(
        endpoint_name, endpoint_type, hub.properties.routing.endpoints
    )
    return client.iot_hub_resource.begin_create_or_update(
        resource_group_name=resource_group_name, resource_name=hub_name, iot_hub_description=hub, if_match=hub.etag
    )
...
def iot_hub_route_delete(cmd, client, hub_name, route_name=None, source_type=None, resource_group_name=None):
    ...
    if route_name:
        hub.properties.routing.routes = [
            route for route in hub.properties.routing.routes if route.name.lower() != route_name.lower()
        ]
    if source_type:
        hub.properties.routing.routes = [
            route for route in hub.properties.routing.routes if route.source.lower() != source_type.lower()
        ]
    return client.iot_hub_resource.begin_create_or_update(
        resource_group_name=resource_group_name, resource_name=hub_name, iot_hub_description=hub, if_match=hub.etag
    )
...
def iot_hub_route_update(
    ...
    hub = iot_hub_get(cmd, client, hub_name, resource_group_name)
    updated_route = next(
        (route for route in hub.properties.routing.routes if route.name.lower() == route_name.lower()), None
    )
    if updated_route:
...
def iot_hub_route_test(
    ...
    resource_group_name = _ensure_hub_resource_group_name(client, resource_group_name, hub_name)
    route_message = RoutingMessage(body=body, app_properties=app_properties, system_properties=system_properties)

    if route_name:
        route = iot_hub_route_show(cmd, client, hub_name, route_name, resource_group_name)
        test_route_input = TestRouteInput(message=route_message, twin=None, route=route)
    ...
    test_all_routes_input = TestAllRoutesInput(routing_source=source_type, message=route_message, twin=None)
...
def iot_central_app_create(
    ...
    app = App(
        subdomain=subdomain, location=location, display_name=display_name, sku=appSku, template=template, identity=appid
    )
...
def _get_iot_central_app_by_name(client, app_name):
    ...
    if all_apps is None:
        raise CLIError("No IoT Central application found in current subscription.")
    try:
        target_app = next(x for x in all_apps if app_name.lower() == x.name.lower())
    except StopIteration:
        raise CLIError("No IoT Central application found with name {} in current subscription.".format(app_name))
    return target_app
...
def get_private_link_resource(client, name=None, connection_id=None, resource_group_name=None, group_id=None):
    ...
    if resource_group_name and name and group_id:
        return client.private_links.get(resource_group_name=resource_group_name, resource_name=name, group_id=group_id)
    if connection_id:
...
def list_private_link_resource(client, app_name=None, connection_id=None, resource_group_name=None, source_type=None):
    if app_name and resource_group_name and source_type:
        if source_type.lower() != "microsoft.iotcentral/iotapps":
            raise InvalidArgumentValueError("Resource type must be Microsoft.IoTCentral/iotApps")
    elif connection_id:
        id_list = connection_id.split("/")
        if id_list[id_list.index("providers") + 1].lower() != "microsoft.iotcentral":
            raise InvalidArgumentValueError("Type must be Microsoft.IoTCentral/iotApps")
        resource_group_name = id_list[id_list.index("resourceGroups") + 1]
...
def show_private_endpoint_connection(
    client, resource_group_name=None, connection_id=None, account_name=None, private_endpoint_connection_name=None
):

    return get_private_endpoint_connection(
        client=client,
        resource_group_name=resource_group_name,
...
def get_private_endpoint_connection(
    ...
    if resource_group_name and account_name and private_endpoint_connection_name:
        output = client.private_endpoint_connections.get(
            resource_group_name=resource_group_name,
...
def _update_private_endpoint_connection_status(
    ...
    getInfoArr = get_private_endpoint_connection(
        client,
        resource_group_name=resource_group_name,
        connection_id=connection_id,
        account_name=account_name,
    ...
    try:
        return client.private_endpoint_connections.begin_create(
        ...
        if ex.response.status_code == 400:
            if new_status == "Approved" and old_status == "Rejected":
                raise CLIError(
                    ex.response,
                    "You cannot approve the connection request after rejection. Please create "
...
def delete_private_endpoint_connection(
    client, resource_group_name=None, account_name=None, private_endpoint_connection_name=None, connection_id=None
):
    ...
    # private_endpoint_connection.private_link_service_connection_state.status = new_status
    # private_endpoint_connection.private_link_service_connection_state.description = description
    return client.private_endpoint_connections.begin_delete(
        resource_group_name=rg, resource_name=acc_name, private_endpoint_connection_name=pec_name
    )
...
def _process_fileupload_args(
    ...
    if (
        ...
        fileupload_storage_authentication_type
        and fileupload_storage_authentication_type == AuthenticationType.KeyBased.value
    ):
    ...
    if hasattr(default_storage_endpoint, "authentication_type"):
        ...
        if (
            default_storage_endpoint.authentication_type == AuthenticationType.IdentityBased.value
            and fileupload_storage_identity
        ):
            # setup new fsi
            default_storage_endpoint.identity = (
                ManagedIdentity(user_assigned_identity=fileupload_storage_identity)
                if fileupload_storage_identity not in [IdentityType.none.value, SYSTEM_ASSIGNED_IDENTITY]
                else None
            )
        # otherwise - let them know they need identity-based auth enabled
        elif fileupload_storage_identity:
            raise ArgumentUsageError(
                "In order to set a file upload storage identity, you must set the file upload storage authentication type (--fsa) to IdentityBased"
            )
...
def _validate_fileupload_identity(instance, fileupload_storage_identity):
    ...
    if not instance_identity or instance_identity == IdentityType.none.value:
        raise ArgumentUsageError(
            "Hub has no identity assigned, please assign a system or user-assigned managed identity to use for file-upload with `az iot hub identity assign`"
        )

    has_system_identity = instance_identity in [
        IdentityType.system_assigned.value,
        IdentityType.system_assigned_user_assigned.value,
    ]
    has_user_identity = instance_identity in [
        IdentityType.user_assigned.value,
        IdentityType.system_assigned_user_assigned.value,
    ]
    ...
    if fileupload_storage_identity in [None, SYSTEM_ASSIGNED_IDENTITY]:
        if not has_system_identity:
            raise ArgumentUsageError(
                "System managed identity must be enabled in order to use managed identity for file upload"
            )
    # if changing to user identity and hub has no user identities
    elif fileupload_storage_identity and not has_user_identity:
        raise ArgumentUsageError(
            "User identity {} must be added to hub in order to use it for file upload".format(
                fileupload_storage_identity
            )
        )
