...
class IoTHubTest(ScenarioTest):
    ...
    def test_iot_hub(self, resource_group, resource_group_location, storage_account):
        ...
        updated_hub = self.cmd('iot hub update -n {0} -g {1} --disable-local-auth --disable-module-sas'.format(hub, rg), expect_failure=True).get_output_in_json()
        ...
        updated_hub = self.cmd('iot hub update -n {0} -g {1} --disable-module-sas false  --disable-device-sas'.format(hub, rg), expect_failure=True).get_output_in_json()
        assert updated_hub['properties']['disableLocalAuth']
        assert updated_hub['properties']['disableDeviceSAS']
        assert not updated_hub['properties']['disableModuleSAS']

        updated_hub = self.cmd('iot hub update -n {0} -g {1} --disable-local-auth false --disable-device-sas false'.format(hub, rg), expect_failure=True).get_output_in_json()
